...
const IconComponent = wrapFieldsWithMeta((props) => {
  ...
  const iconName = props.input.value as unknown as keyof typeof lu;
  ...
  const filteredIcons = Object.keys(lu).filter((iconKey) =>
  ...
  const handleIconSelect = (name: keyof typeof lu) => {
  ...
  return (
    <div>
      <div style={{ marginBottom: "1rem" }}>
        ...
        {iconName ? React.createElement(lu[iconName], { size: 24 }) : "None"}
      </div>
            ...
            return (
              ...
              >
                {filteredIcons.slice(startIndex, endIndex).map((iconKey) => {
                  const Icon = lu[iconKey as keyof typeof lu];
                  const isSelected = iconKey === iconName;
                  return (
                    <div
                      key={iconKey}
                      onClick={() =>
                        handleIconSelect(iconKey as keyof typeof lu)
                      }
                      style={{
                        cursor: "pointer",
                        padding: "5px",
